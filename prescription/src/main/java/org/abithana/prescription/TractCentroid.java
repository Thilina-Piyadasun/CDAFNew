...
public class TractCentroid implements Serializable{
    ...
    public List<TractCentroidBean> getAllTractCentroids(String tableName){
        ...
        List<Integer> allTractIDList=getAllTractID(tableName);
        ...
        for(int tractID:allTractIDList){
    ...
    public List<Integer> getAllTractID(String tableName){

        DataFrame df=instance.getSqlContext().sql("select tractID from "+tableName).distinct();

        List<Integer> tractIDList = df.javaRDD().map(new Function<Row, Integer>() {
            public Integer call(Row row) {

                int tractID=row.getAs("tractID");
                return tractID;
            }
        }).collect();
    ...
    public TractCentroidBean getBestLocation(int tractID,String tableName){
